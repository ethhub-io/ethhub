...
Tezos, the first blockchain to implement a Liquid Proof of Stake (LPoS) consensus mechanism, launched on June 30, 2018 and is the strongest example of a proof of stake blockchain that has been tested and validated at scale. Like Ethereum, it intends to maximize decentralization, and is already accomplishing that with a robust and diverse community of validators that continues to grow (https://tzscan.io/rolls-distribution). It is recommended that one check out this article for an overview of LPoS and how it differs from DPoS: https://medium.com/tezos/liquid-proof-of-stake-aec2f7ef1da7.

There are currently multiple projects that employ the Proof of Stake consensus mechanism. In addition to Tezos, Peercoin and NXT are two more blockchains in the wild that utilize Proof of Stake. There have been many lessons learned from these deployments including potential attack vectors, such as the "nothing at stake" problem. Ethereum researchers have taken these lessons learned and incorporated them into their design requirements. For example, Casper Proof of Stake addresses the nothing at stake problem by enforcing a set of pre-defined conditions and rules that if violated by a malicious party will result in the "slashing" of their staked Ether as a penalty. For example, if a validator proposed a different version of the blockchain versus what the remaining honest nodes have reached consensus on, that validator is slashed a portion of their staked Ether as a punitive measure. This discourages attacks as their stake ironically becomes something at stake. 
